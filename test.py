...
for i in range(len(k)):
    ...
    if r ==0 : pred_labels.append("actinicKeratosis")
    elif r ==1: pred_labels.append("dermatofibroma")
    elif r ==2: pred_labels.append("nevus")	
...
for m in range(1, columns*rows +1):
    ...
    if pred_labels[m-1] =='actinicKeratosis':
        plt.imshow(img)
        cv2.rectangle(img, (l[m-1][0], l[m-1][1]), (l[m-1][2], l[m-1][3]), (0,255,0), 2)
    elif pred_labels[m-1] =='dermatofibroma':
        plt.imshow(img)
